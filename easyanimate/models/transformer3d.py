...
class HunyuanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if clip_encoder_hidden_states is not None:
            batch_size = clip_encoder_hidden_states.shape[0]
            clip_encoder_hidden_states = self.clip_projection(clip_encoder_hidden_states)
            
            clip_encoder_hidden_states = clip_encoder_hidden_states.view(batch_size, -1, clip_encoder_hidden_states.shape[-1])
        ...
        for layer, block in enumerate(self.blocks):
            ...
            # Add Pose embedding 
            if pose_emb is not None and layer == 0:
                hidden_states = hidden_states + pose_emb
