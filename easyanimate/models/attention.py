...
class HunyuanDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm_temporal = FP32LayerNorm(dim, norm_eps, norm_elementwise_affine)
    ...
    def forward(
        ...
        if num_frames != 1:
            ...
            if self.t_embed is not None:
                # XXX: the first frame is the condition is not added time embedding
                cond_hidden_states = hidden_states[:, 0:1] # the first frame
        ...
        if self.attn_clip is not None and clip_encoder_hidden_states is not None:
